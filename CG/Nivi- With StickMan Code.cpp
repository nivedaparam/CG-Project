...
void intro1()
{
	...
	bitmap_output(-0.2,0, "Days of Future Past", GLUT_BITMAP_TIMES_ROMAN_24);
...
void display3(void) 
{
	glClear(GL_COLOR_BUFFER_BIT);
		glColor3f(1,1,1);
	bitmap_output(-0.9, 0, "Bezelless Display To", GLUT_BITMAP_TIMES_ROMAN_24);
...
void display4(void) 
{
	...
	glEnd();
	glBegin(GL_QUADS);                                                                      /*Change the green quad into polygon to  make the notch and delete */
	glColor3f(0,0,0);									/*other two quads*/
	glVertex2f(-.375,-.775);																	
	glVertex2f(.375,-.775);
	glVertex2f(.375, .775);
	glVertex2f(-.375, .775);
